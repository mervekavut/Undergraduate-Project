...
<div class="content-wrapper">
        ...
        <section class="content">
            ...
            if ($_POST)
            {
                if (!empty($_POST["telefon"]) && !empty($_POST["mail"]) && !empty($_POST["adres"]) && !empty($_POST["fax"]))
                {
                    ...
                    $fax=$VT->filter ($_POST["fax"]);
                          ...
                          $güncelle=$VT->SorguCalistir ("UPDATE ayarlar" ,"SET telefon=?, mail=?, adres=?, fax=? WHERE ID=?", array($telefon, $mail, $adres, $fax,1 ),1);
                    
                                                      
                    if ($güncelle!=false)
                    {
                        ?>
                        <meta http-equiv="refresh" content="2;url=<?=SITE?>iletisim-ayarlari" /> <!--işlem kaydedildikten sonra iletişim ayarları sayfasına gönderiyor-->
            ...
            <div class="col-md-12">
                ...
                <!--value kısmı başlık kısmında içi var olan başlık yazılı gelmesi için kullanıldı -->
                </div>
            </div>
