<?php
//php içerisinde if bölümü açıldı
if (!empty($_GET["tablo"])) {
...
?>
                ...
                <div class="container-fluid">
                    ...
                    if ($_POST) {
                        if (!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"])) {
                            ...
                            $resimadı=$_FILES["resim"]["name"];
                            if (!empty($resimadı)) {
                                $yukle = $VT->upload("resim", "../images/" . $kontrol[0]["tablo"] . "/");

                                if ($yukle ) {
                                    $ekle = $VT->SorguCalistir("INSERT INTO " . $kontrol[0]["tablo"],
                                     " SET baslik=?, seflink=?, kategori=?, metin=?, resim=?, anahtar=?, description=?,durum=?, sirano=?, tarih=?", array($baslik, $seflink, $kategori, $metin, $resimadı, $anahtar, $description, 1, $sirano, date("Y-m-d")));
                                } else {
                    ?>
                                ...
                                $ekle = $VT->SorguCalistir("INSERT INTO " . $kontrol[0]["tablo"],
                                 "SET baslik=?, seflink=?, kategori=?, metin=?, anahtar=?, description=?, durum=?, sirano=?, tarih=?", array($baslik, $seflink, $kategori, $metin, $anahtar, $description, 1, $sirano, date("Y-m-d")));
                                if ($ekle) {
                                ?>
    ...
    <?php
        //php içerisinde if bölümü kapatıldı
    } else {
    ?>
