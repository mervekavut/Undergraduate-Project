        ...
        <section class="content">
            ...
            if ($_POST)
            {
                if (!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"]))
                {
                    ...
                    $resim="../images/".$kontrol[0]["tablo"]."/";
                    $resimadı=$_FILES["resim"]["name"];
                    if (!empty($resimadı))
                    {
                          $yukle=$VT->upload ("resim",$resim);
                          if ($yukle)
                          {
                            $ekle=$VT->SorguCalistir ("UPDATE ".$kontrol[0]["tablo"],
                            " SET baslik=?, seflink=?, kategori=?, metin=?, resim=?, anahtar=?, description=?,durum=?, sirano=?, tarih=?  WHERE ID=?", 
                            array($baslik, $seflink, $kategori, $metin, $resimadı, $anahtar, $description, 1, $sirano, date ("Y-m-d"),$veri[0]["ID"]));
                          }
                          else
                          {
                                ?>
                    ...
                    {
                        $ekle=$VT->SorguCalistir ("UPDATE ".$kontrol[0]["tablo"],
                        "SET baslik=?, seflink=?, kategori=?, metin=?, anahtar=?, description=?, durum=?, sirano=?, tarih=?, WHERE ID=?", 
